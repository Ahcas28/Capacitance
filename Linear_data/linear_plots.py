...
def extract_params(filename):
    ...
    match = re.search(r'_N(\d+)_r(\d+)_gamma(\d+)', filename)
    if match:
        R = int(match.group(1))
...
def plot_real_eigenfrequencies(eigs,params):
    ...
    plt.ylabel(r"Eigenvalue $\lambda_n$")
...
def plot_modes(matrice,params):
    ...
    plt.ylabel(r"Mode value $\psi_k(n)$")
...
def  plot_IPR(modes, list):
    ...
    for i, mode in enumerate(modes.values()):
        ...
        for j in range(0,size):
            ...
            IPR = compute_IPR(mod)
            IPRs.append(IPR)
            plt.scatter(list[i], IPR, color = colors[i], marker="*", s=8)
        avg = np.mean(IPRs)
        avgs.append(avg)
